...
class UpstreamClientTest {
    ...
    @Test
    void outputPortBuildsHeadersAndPreservesUpstreamFailure() {
        var route = new com.example.gateway.domain.routing.model.ApiRoute(
                "org", "service", "api", "POST", "http://localhost", "/conflict",
                "test-key", "test-api-key", "test-institution", true);
        server.expect(MockRestRequestMatchers.requestTo("http://localhost/conflict"))
                .andExpect(MockRestRequestMatchers.header("X-Checksum", "checksum"))
                .andExpect(MockRestRequestMatchers.header("X-Api-Key", "test-api-key"))
                .andExpect(MockRestRequestMatchers.header("X-Ins-Code", "test-institution"))
                .andExpect(MockRestRequestMatchers.content().json("{\"data\":\"cipher\"}"))
                .andRespond(MockRestResponseCreators.withStatus(HttpStatus.CONFLICT).body("conflict"));

        var result = upstreamClient.post(route, URI.create("http://localhost/conflict"),
                "checksum", Map.of("data", "cipher"));

        assertThat(result.status()).isEqualTo(409);
        assertThat(result.body()).isEqualTo("conflict");
        server.verify();
    }
