...
class ReportViewTest {
    ...
    @Test
    void apiFiltersPreserveJsonContractAndRejectInvalidDates() throws Exception {
        mvc.perform(get("/api/runs").param("org", "orgA").param("service", "reservation").param("status", "FAIL")
                        .param("from", "2026-09-25T00:00"))
                .andExpect(status().isOk())
                .andExpect(org.springframework.test.web.servlet.result.MockMvcResultMatchers.jsonPath("$[0].id").value("ui-view-test"))
                .andExpect(org.springframework.test.web.servlet.result.MockMvcResultMatchers.jsonPath("$[0].startedAt").value("2026-09-25T10:30:00"));
        mvc.perform(get("/api/runs").param("from", "invalid")).andExpect(status().isBadRequest());
    }

    @Test
    void rawReportLinkIncludesServletContextPath() throws Exception {
        mvc.perform(get("/reports-app/runs/ui-view-test").contextPath("/reports-app"))
                .andExpect(status().isOk())
                .andExpect(content().string(containsString("href=\"/reports-app/reports/ui-view-test/report/karate-summary.html\"")));
    }
    ...
    @Test
    void servicesApiPreservesTimestampFormatAndFields() throws Exception {
        mvc.perform(get("/api/services"))
                .andExpect(status().isOk())
                .andExpect(content().string(containsString("\"lastRunAt\":\"2026-09-25T10:30:00\"")))
                .andExpect(content().string(containsString("\"lastRunId\":\"ui-view-test\"")));
    }
