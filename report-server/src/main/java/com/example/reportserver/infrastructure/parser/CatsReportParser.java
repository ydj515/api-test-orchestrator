...
import com.example.reportserver.infrastructure.parser.dto.CatsIndividualReport;
import com.example.reportserver.infrastructure.parser.dto.CatsSummaryReport;
...
public class CatsReportParser {
    ...
    private LocalDateTime resolveStartedAt(Path reportDir) {
        ...
        try (Stream<Path> files = Files.list(reportDir)) {
            return files
                    ...
                    .orElseGet(() -> {
                        log.warn("CATS report has no usable execution timestamp: {}", reportDir);
                        return null;
                    });
        } catch (IOException e) {
            throw new UncheckedIOException("Failed to list CATS report: " + reportDir, e);
        }
    }
    ...
    private long parseDurationMs(String timeToExecuteInMs) {
        ...
        try {
            long duration = Long.parseLong(timeToExecuteInMs.trim());
            if (duration < 0) {
                throw new IllegalArgumentException("CATS duration must not be negative");
            }
            return duration;
        } catch (NumberFormatException e) {
            throw new IllegalArgumentException("Invalid CATS duration: " + timeToExecuteInMs, e);
        }
    }
}
